...
// Use a proper type when converting pointers to integer values for indexing
#if UINTPTR_MAX == 0xffffffff  // 32 bits
    #define POINTER_TO_INT_TYPE unsigned int
#elif UINTPTR_MAX == 0xffffffffffffffff  // 64 bits
    #define POINTER_TO_INT_TYPE unsigned long long
...
static	sAllocUnit	*findAllocUnit(const void *reportedAddress)
{
	...
	unsigned int	hashIndex = (reinterpret_cast<POINTER_TO_INT_TYPE>(const_cast<void *>(reportedAddress)) >> 4) & (hashSize - 1);
...
static	void	dumpAllocations(FILE *fp)
{
	...
	for (unsigned int i = 0; i < hashSize; i++)
	{
		...
		while(ptr)
		{
			...
			fprintf(fp, "%06d 0x%llX 0x%llX     0x%llX 0x%llX     0x%08X %-8s    %c       %c    %s\r\n",
#else
			fprintf(fp, "%06d 0x%8X    0x%04X   0x%08X    0x%04X   0x%08X %-8s    %c       %c    %s\r\n",
#endif
				ptr->allocationNumber,
				reinterpret_cast<POINTER_TO_INT_TYPE>(ptr->reportedAddress), ptr->reportedSize,
				reinterpret_cast<POINTER_TO_INT_TYPE>(ptr->actualAddress), ptr->actualSize,
				mmgr_calcUnused(ptr),
...
bool	&mmgr_breakOnRealloc(void *reportedAddress)
{
...
bool	&mmgr_breakOnDealloc(void *reportedAddress)
{
	// Locate the existing allocation unit
...
void	mmgr_setOwner(const char *file, const unsigned int line, const char *func)
{
...
void	*operator new(size_t reportedSize)
{
	...
	for(;;)
	{
		...
		void	*ptr = mmgr_allocator(file, line, func, m_alloc_new, reportedSize);
		if (ptr)
		{
			#ifdef TEST_MEMORY_MANAGER
			log("[D] EXIT : new");
...
void	*operator new[](size_t reportedSize)
{
	...
	for(;;)
	{
		...
		void	*ptr = mmgr_allocator(file, line, func, m_alloc_new_array, reportedSize);
		if (ptr)
		{
			#ifdef TEST_MEMORY_MANAGER
			log("[D] EXIT : new[]");
...
void	*operator new(size_t reportedSize, const char *sourceFile, int sourceLine)
{
	...
	for(;;)
	{
		...
		void	*ptr = mmgr_allocator(sourceFile, sourceLine, "??", m_alloc_new, reportedSize);
		if (ptr)
		{
			#ifdef TEST_MEMORY_MANAGER
			log("[D] EXIT : new");
...
void	*operator new[](size_t reportedSize, const char *sourceFile, int sourceLine)
{
	...
	for(;;)
	{
		...
		void	*ptr = mmgr_allocator(sourceFile, sourceLine, "??", m_alloc_new_array, reportedSize);
		if (ptr)
		{
			#ifdef TEST_MEMORY_MANAGER
			log("[D] EXIT : new[]");
...
void	operator delete(void *reportedAddress)
{
	...
	if (reportedAddress) mmgr_deallocator(sourceFile, sourceLine, sourceFunc, m_alloc_delete, reportedAddress);
...
void	operator delete[](void *reportedAddress)
{
	...
	if (reportedAddress) mmgr_deallocator(sourceFile, sourceLine, sourceFunc, m_alloc_delete_array, reportedAddress);
...
void	*mmgr_allocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int allocationType, const size_t reportedSize)
{
	try
	{
		#ifdef TEST_MEMORY_MANAGER
		log("[D] ENTER: mmgr_allocator()");
		#endif
		...
		POINTER_TO_INT_TYPE	hashIndex = (reinterpret_cast<POINTER_TO_INT_TYPE>(au->reportedAddress) >> 4) & (hashSize - 1);
		...
		if (alwaysValidateAll) mmgr_validateAllAllocUnits();

		// Log the result

		if (alwaysLogAll) log("[+] ---->             addr 0x%08X", reinterpret_cast<POINTER_TO_INT_TYPE>(au->reportedAddress));
...
void	*mmgr_reallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int reallocationType, const size_t reportedSize, void *reportedAddress)
{
	try
	{
		#ifdef TEST_MEMORY_MANAGER
		log("[D] ENTER: mmgr_reallocator()");
		#endif
		...
		if (!reportedAddress)
		{
			return mmgr_allocator(sourceFile, sourceLine, sourceFunc, reallocationType, reportedSize);
		}
		...
		m_assert(mmgr_validateAllocUnit(au));
		...
		if (oldReportedAddress != au->reportedAddress)
		{
			...
			{
				unsigned int	hashIndex = (reinterpret_cast<POINTER_TO_INT_TYPE>(oldReportedAddress) >> 4) & (hashSize - 1);
			...
			hashIndex = (reinterpret_cast<POINTER_TO_INT_TYPE>(au->reportedAddress) >> 4) & (hashSize - 1);
			if (hashTable[hashIndex]) hashTable[hashIndex]->prev = au;
			au->next = hashTable[hashIndex];
			au->prev = NULL;
			hashTable[hashIndex] = au;
...
void	mmgr_deallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int deallocationType, const void *reportedAddress)
{
	try
	{
		#ifdef TEST_MEMORY_MANAGER
		log("[D] ENTER: mmgr_deallocator()");
		#endif

		// Log the request

		if (alwaysLogAll) log("[-] ----- %8s of addr 0x%08X           by %s", allocationTypes[deallocationType], reinterpret_cast<POINTER_TO_INT_TYPE>(const_cast<void *>(reportedAddress)), ownerString(sourceFile, sourceLine, sourceFunc));
		...
		if (reportedAddress)
		{
			...
			unsigned int	hashIndex = (reinterpret_cast<POINTER_TO_INT_TYPE>(au->reportedAddress) >> 4) & (hashSize - 1);
			if (hashTable[hashIndex] == au)
			{
...
bool	mmgr_validateAllocUnit(const sAllocUnit *allocUnit)
{
...
bool	mmgr_validateAllAllocUnits()
{
	...
	for (unsigned int i = 0; i < hashSize; i++)
	{
		...
		{
			allocCount++;
			if (!mmgr_validateAllocUnit(ptr)) errors++;
			ptr = ptr->next;
		}
	}
...
unsigned int	mmgr_calcUnused(const sAllocUnit *allocUnit)
{
...
unsigned int	mmgr_calcAllUnused()
{
...
void	mmgr_dumpAllocUnit(const sAllocUnit *allocUnit, const char *prefix)
{
...
void	mmgr_dumpMemoryReport(const char *filename, const bool overwrite)
{
	// Open the report file
...
sMStats	mmgr_getMemoryStatistics()
{
	return stats;
}
